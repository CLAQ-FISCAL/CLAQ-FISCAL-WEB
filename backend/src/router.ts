...
const routes: Route[] = [
  ...
  {
    method: 'POST',
    pattern: /^\/v1\/payments\/mpesa$/,
    status: 201,
    handler: async (ctx) => {
      const { MpesaService } = await import('./modules/payments/mpesa/mpesa.service.js');
      const customerMsisdn = requireString(ctx.body, 'customerMsisdn', 20);
      const amount = Number(ctx.body.amount);
      if (!Number.isFinite(amount) || amount <= 0) throw new AppError(422, 'INPUT_INVALID', "Field 'amount' must be a positive number.");
      const transactionReference = requireString(ctx.body, 'transactionReference', 100);
      const thirdPartyReference = optionalString(ctx.body, 'thirdPartyReference', 100) || `CLAQ_${Date.now()}`;

      const result = await MpesaService.initiateC2BPayment({ customerMsisdn, amount, transactionReference, thirdPartyReference });
      ...
      await ctx.run(async (tx) => {
        await tx.paymentTransaction.create({
          data: {
            ...
            metadata: { conversationId: result.conversationId, responseCode: result.responseCode },
          },
        });
      });

      return { provider: 'MPESA', ...result };
    },
  },
  {
    method: 'POST',
    pattern: /^\/v1\/payments\/emola$/,
    status: 201,
    handler: async (ctx) => {
      const { EmolaService } = await import('./modules/payments/emola/emola.service.js');
      const phone = requireString(ctx.body, 'phone', 20);
      const amount = Number(ctx.body.amount);
      if (!Number.isFinite(amount) || amount <= 0) throw new AppError(422, 'INPUT_INVALID', "Field 'amount' must be a positive number.");
      const orderId = optionalString(ctx.body, 'orderId', 100) || `EMOLA_${Date.now()}`;

      const result = await EmolaService.initiateC2BPayment({ phone, amount, orderId });
      ...
      return { provider: 'EMOLA', ...result };
    },
  },
  {
    method: 'POST',
    pattern: /^\/v1\/payments\/simo$/,
    status: 201,
    handler: async (ctx) => {
      const { SimoCardService } = await import('./modules/payments/simo/simo.service.js');
      const cardNumber = requireString(ctx.body, 'cardNumber', 20);
      const cardHolder = requireString(ctx.body, 'cardHolder', 200);
      const expiryMonth = requireString(ctx.body, 'expiryMonth', 2);
      const expiryYear = requireString(ctx.body, 'expiryYear', 4);
      const cvv = requireString(ctx.body, 'cvv', 4);
      const amount = Number(ctx.body.amount);
      if (!Number.isFinite(amount) || amount <= 0) throw new AppError(422, 'INPUT_INVALID', "Field 'amount' must be a positive number.");
      const currency = (ctx.body.currency as string) === 'USD' ? 'USD' : 'MZN';

      const result = await SimoCardService.processCardPayment({ cardNumber, cardHolder, expiryMonth, expiryYear, cvv, amount, currency });
      ...
      await ctx.run(async (tx) => {
        await tx.paymentTransaction.create({
          data: {
            ...
            amount,
            currency,
            metadata: { authorizationCode: result.authorizationCode, cardBrand: result.cardBrand },
          },
        });
      });

      return { provider: 'SIMO', ...result };
    },
  },
  {
    method: 'POST',
    pattern: /^\/v1\/payments\/bank-transfer$/,
    status: 201,
    handler: async (ctx) => {
      const { BankTransferService } = await import('./modules/payments/bank/bankTransfer.service.js');
      const companyNuit = requireString(ctx.body, 'companyNuit', 9);
      if (!/^\d{9}$/.test(companyNuit)) throw new AppError(422, 'INPUT_INVALID', 'NUIT must be exactly 9 digits.');
      const invoiceId = requireString(ctx.body, 'invoiceId', 100);
      const amount = Number(ctx.body.amount);
      if (!Number.isFinite(amount) || amount <= 0) throw new AppError(422, 'INPUT_INVALID', "Field 'amount' must be a positive number.");

      const result = BankTransferService.generatePaymentReference({ companyNuit, invoiceId, amount });
      ...
      await ctx.run(async (tx) => {
        await tx.paymentTransaction.create({
          data: {
            ...
            externalId: result.referenceNumber,
            amount,
            description: `Bank transfer for invoice ${invoiceId}`,
            metadata: { entityCode: result.entityCode, expiryDate: result.expiryDate, banks: result.supportedBanks },
          },
        });
      });

      return { provider: 'BANK_TRANSFER', ...result };
    },
  },
  {
    method: 'GET',
    ...
    handler: async (ctx) => {
      ...
      if (!Number.isInteger(limitRaw) || limitRaw < 1 || limitRaw > 200) throw new AppError(422, 'INPUT_INVALID', "'limit' must be an integer between 1 and 200.");
      return ctx.run(async (tx) => {
        const items = await tx.paymentTransaction.findMany({
          where: { tenantId: ctx.tenantId, ...(ctx.query.provider ? { provider: ctx.query.provider as any } : {}), ...(ctx.query.status ? { status: ctx.query.status as any } : {}) },
          orderBy: { createdAt: 'desc' },
          take: limitRaw,
        });
        return { items };
  ...
  {
    method: 'POST',
    pattern: /^\/v1\/payments\/webhook\/mpesa$/,
    status: 200,
    handler: async (ctx) => {
      const { MpesaService } = await import('./modules/payments/mpesa/mpesa.service.js');
      const rawBody = JSON.stringify(ctx.body);
      const signatureHeader = optionalString(ctx.body, 'output_ResponseID', 200);
      const webhookSecret = process.env.MPESA_WEBHOOK_SECRET;
      const result = await MpesaService.processWebhookCallback(ctx.body, signatureHeader, rawBody, webhookSecret);

      if (result.success && result.transactionId) {
        await ctx.run(async (tx) => {
          const payment = await tx.paymentTransaction.findFirst({ where: { externalId: result.transactionId } });
          if (payment) {
            await tx.paymentTransaction.update({ where: { id: payment.id }, data: { status: 'COMPLETED', webhookVerified: true } });
          }
        });
      }
  ...
  // ---------- AI / RAG ----------
  {
    ...
    status: 201,
    handler: async (ctx) => {
      const { AIRagService } = await import('./modules/ai/aiRag.service.js');
      const prompt = requireString(ctx.body, 'prompt', 2000);
      const contextSimulatorId = optionalString(ctx.body, 'contextSimulatorId', 50);
      const userRole = ctx.role;

      const result = await AIRagService.queryTaxIntelligence({ prompt, contextSimulatorId, userRole });
      return result;
    },
  },
  // ---------- PDF Generation ----------
  {
    method: 'POST',
    pattern: /^\/v1\/pdf\/generate$/,
    status: 201,
    handler: async (ctx) => {
      const { PDFGeneratorService } = await import('./modules/pdf/pdfGenerator.service.js');
      const simulationId = requireString(ctx.body, 'simulationId', 64);
      const clientName = requireString(ctx.body, 'clientName', 200);
      const clientNuit = requireString(ctx.body, 'clientNuit', 9);
      const totalTaxAmount = Number(ctx.body.totalTaxAmount);
      if (!Number.isFinite(totalTaxAmount) || totalTaxAmount < 0) throw new AppError(422, 'INPUT_INVALID', "Field 'totalTaxAmount' must be a non-negative number.");
      const currency = optionalString(ctx.body, 'currency', 10) || 'MZN';
      const responsibleName = requireString(ctx.body, 'responsibleName', 200);
      const calculationBreakdown = typeof ctx.body.calculationBreakdown === 'object' && ctx.body.calculationBreakdown !== null ? ctx.body.calculationBreakdown as Record<string, any> : {};

      const result = await PDFGeneratorService.generateAndUploadSignedCertificate({ simulationId, clientName, clientNuit, totalTaxAmount, currency, responsibleName, calculationBreakdown });
      return result;
    },
  },
  // ---------- Exchange Rates ----------
  {
    ...
    handler: async (ctx) => {
      return ctx.run(async (tx) => {
        const base = ctx.query.base || 'USD';
        const rates = await tx.exchangeRateCache.findMany({
          where: { base },
          orderBy: { fetchedAt: 'desc' },
          take: 20,
        });
        return { base, rates: rates.map((r) => ({ target: r.target, rate: r.rate.toString(), source: r.source, fetchedAt: r.fetchedAt.toISOString() })) };
      });
    },
  },
  // ---------- News ----------
  {
    ...
    handler: async (ctx) => {
      ...
      return ctx.run(async (tx) => {
        const items = await tx.newsArticle.findMany({
  ...
  {
    method: 'POST',
    pattern: /^\/v1\/newsletter\/subscribe$/,
    status: 201,
    handler: async (ctx) => {
      const email = requireString(ctx.body, 'email', 200);
      if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) throw new AppError(422, 'INPUT_INVALID', 'Invalid email address.');
      return ctx.run(async (tx) => {
        const existing = await tx.newsletterSubscriber.findUnique({ where: { email } });
        if (existing) {
          if (!existing.active) {
            await tx.newsletterSubscriber.update({ where: { email }, data: { active: true } });
          }
